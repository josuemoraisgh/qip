...
\title{\textbf{Metodologia de Preparação, Síntese e Validação de Dados (QIP) -- Versão Revisada}}
\author{Resia Silva de Morais}
\date{\today}
...
\section{Visão geral}
A metodologia adotada baseia-se em: (i) preparar a base real; (ii) calibrar uma heurística de decisão multi-classe; (iii) sintetizar dados condicionais por classe com um cWGAN\textendash GP; (iv) treinar, \textbf{exclusivamente com os dados sintéticos gerados na etapa anterior}, um modelo heurístico baseado em \emph{softmax}; e (v) validar o desempenho em um conjunto externo não utilizado em nenhuma etapa anterior. A abordagem foi concebida para cenários de amostras escassas e classes desbalanceadas, garantindo rastreabilidade e separação rigorosa entre \emph{dados para análise} e \emph{dados para validação}.
...
Adota-se o critério de \textbf{não utilizar variáveis demográficas} e/ou outras perguntas cuja influência seja difícil de analisar por conhecimentos heurísticos. Essa filtragem reduz dimensionalidade e preserva apenas atributos com contribuição informativa prévia. Regras adicionais de exclusão podem ser incorporadas (por exemplo, uma \emph{regra de ouro} baseada em pontuações).
...
\section{Treinamento final (softmax/heurística) com \textbf{dados sintéticos} de A}
O modelo heurístico final é treinado \textbf{somente sobre os dados sintéticos} produzidos a partir da metade A, mantendo as mesmas restrições de projeto (máscara de atributos, projeção $[-1,1]$, \emph{early stopping}). Essa etapa consolida um classificador \emph{softmax} que generaliza a partir de exemplos gerados.

\section{Validação externa e análises estatísticas}
A heurística final é aplicada \textbf{exclusivamente} na metade B (jamais vista). Em seguida, são conduzidas as análises estatísticas e de qualidade: macro top-$k$ (para $k=1,2,3$), métricas por classe (acerto/suporte), bem como \emph{KS}, diferença de proporções de classe e \emph{gap} de correlação quando pertinente.

\section{Fluxograma do processo (versão revisada)}
\begin{figure}[h!]
\centering
\begin{tikzpicture}[
    ...
    dashedbox/.style={draw, dashed, rounded corners=2pt, align=center, minimum width=44mm, minimum height=8mm, fill=gray!3}
]

% Entrada
\node[io] (start) {Respostas do \\Questionário Interativo Psicopatológico - QIP};
...
\node[box, below=of aug] (heur) {Treinamento final\\ (softmax/heurística $W\in[-1,1]$)\\ \textbf{usando sintéticos (A)}};
...
% Conexão do modelo treinado até a validação (indicativa)
\draw[dashed,->] (heur.east) -- ++(10mm,0) |- node[pos=0.25, right]{\footnotesize modelo final} (test.west);

% Caixa de análises estatísticas (após validação)
\node[box, below=16mm of test] (metrics) {Análises estatísticas\\ (KS, correlação, proporções)};

% Setas para métricas (apenas após validação)
\draw[->] (test) -- (metrics);
...
\end{tikzpicture}
\caption{Fluxograma revisado: o treinamento final usa \textbf{apenas sintéticos (A)} e a validação ocorre na \textbf{Metade B}; as análises estatísticas são realizadas \textbf{após} a validação.}
\end{figure}
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{Avaliação}: frequência de avaliação e relatórios automáticos.
\end{itemize}
