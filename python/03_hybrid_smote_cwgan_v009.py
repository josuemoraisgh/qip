...
def main():
    ap = argparse.ArgumentParser(description="Hybrid SMOTE + cWGAN-GP (Golden-Only Features)")
    ...
    ap.add_argument("--golden-only", action=argparse.BooleanOptionalAction, default=True,
                    help="Usa APENAS as features presentes em 'Pontuação' com soma != 0. Exclui features fora de Pontuação e as de soma zero.")
    ...
    if args.golden_only and pontos_df is not None:
        # Qualquer coluna presente em ambas (e não é target) é candidata
        numeric_cols = df_full.select_dtypes(include=[np.number]).columns.tolist()
        if target in numeric_cols:
            numeric_cols.remove(target)

        # Interseção com Pontuação
        common = [c for c in pontos_df.columns if c in numeric_cols and c != target]

        # Avalia soma absoluta na Pontuação
        for c in common:
        ...
        # Features numéricas que NÃO aparecem na Pontuação serão excluídas
        features_not_in_pont = [c for c in numeric_cols if c not in common]

        # Mantém só as aprovadas pela regra de ouro
        df_full = df_full.drop(columns=features_not_in_pont + features_zero_in_pont, errors='ignore')
        print(f"[INFO] GOLDEN-ONLY ativo.")
        print(f"       - Numéricas totais: {len(numeric_cols)}")
    ...
    print(f"[INFO] Features finais (GOLDEN-ONLY): {len(feat_cols)}")
