...
def main():
    ...
    ap.add_argument("--drop-zero-by-pontuacao", action=argparse.BooleanOptionalAction, default=True,
                    help="Regra de ouro: se na aba 'Pontuação' uma coluna for toda zero, excluir a coluna correspondente de X.")
    ...
    # ---------- REGRA DE OURO: excluir colunas de X se 'Pontuação' tiver toda zero ----------
    dropped_by_pont = []
    if args.drop_zero_by_pontuacao and pontos_df is not None:
        # Considera apenas colunas compartilhadas e numéricas em 'Pontuação'
        common = [c for c in pontos_df.columns if c in df_full.columns and c != target]
        for c in common:
            s = pd.to_numeric(pontos_df[c], errors='coerce').fillna(0.0)
            if float(s.abs().sum()) == 0.0:
                dropped_by_pont.append(c)
        if dropped_by_pont:
            print(f"[INFO] (Pontuação) Colunas com todos os valores zero serão EXCLUÍDAS de X: {len(dropped_by_pont)}")
            for c in dropped_by_pont[:50]:
                print("   -", c)
            # remove do df_full (somente features, não mexe na aba Pontuação)
            df_full = df_full.drop(columns=[c for c in dropped_by_pont if c in df_full.columns], errors='ignore')
    # ----------------------------------------------------------------------------------------

    # Mostrar contagem por classe (após os drops) e remover classes raras
    counts_full = df_full[target].value_counts().sort_index()
    print("[INFO] Contagem por classe (após pré-processamento inicial):")
    ...
    # Seleciona colunas numéricas como features (após remover por Pontuação)
    ...
    if dropped_by_pont:
        # Confirma que foram realmente removidas das features
        efetivamente = [c for c in dropped_by_pont if c not in feat_cols]
        print(f"[INFO] Features efetivamente removidas por 'Pontuação'=0: {len(efetivamente)}")
    ...
    # Oversampling
    def fit_resample_balanced_local(X, y, n_classes, seed=42, target_total=5400):
        X_res, y_res = X, y
        used = []
        counts = np.bincount(y, minlength=n_classes)
        pos_counts = counts[counts > 0]
    ...
    X_sm, y_sm, over_used = fit_resample_balanced_local(X_all, y_all, n_classes, seed=args.seed, target_total=args.smote_target)
    ...
    with pd.ExcelWriter(out_xlsx, engine='openpyxl') as writer:
        ...
        # Excluídas por Pontuação=0 (lista de colunas)
        if dropped_by_pont:
            pd.DataFrame({"dropped_by_pontuacao_zero": dropped_by_pont}).to_excel(
                writer, index=False, sheet_name="FEATURES_DROPPED_BY_PONTUACAO"
            )
        # Report simples
        rep = pd.DataFrame({
