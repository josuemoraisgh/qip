...
def fit_resample_balanced(X, y, n_classes, seed=42, target_total=5400):
    ...
    if cur_total < target_total:
        ...
        # distribute approximately uniform, favoring classes with menores contagens atuais
...
class CWGAN_GP:
    ...
    def train(self, loader, n_classes, epochs=2000, log_every=100,
        ...
        for ep in range(1, epochs + 1):
            ...
            # reset last-known metrics for this epoch
            ...
            for x_real, y_idx in loader:
                ...
                if user_pressed_q_nonblocking():
                    print("[INFO] Stop solicitado pelo usuário ('q'). Encerrando treino antecipadamente.", flush=True)
                    ep = epochs  # ensure outer loop ends
                    break
...
def main():
    ap = argparse.ArgumentParser(description="Hybrid SMOTE + cWGAN-GP (v011: MinMax, Inteiros 0..10, GOLDEN, log robusto)")
    ...
    ap.add_argument("--inst-noise", type=float, default=0.0,
    ...
    # ---------- GOLDEN-ONLY: selecionar features APENAS se presentes em 'Pontuação' E com soma != 0 ----------
    ...
    if df_holdout is not None and len(df_holdout) > 0:
        X_hold_orig = df_holdout[feat_cols].to_numpy(dtype=np.float32)
        y_hold = df_holdout[target].astype(str).str.strip().values
    else:
        X_hold_orig = None
        y_hold = None
    ...
    # Eval set (em original -> escala e depois avaliamos em ORIGINAL)
    if df_holdout is not None and len(df_holdout) > 0:
        X_eval_orig = X_hold_orig.copy()
        y_eval_idx = pd.Series(y_hold).map(label_to_idx).to_numpy(dtype=np.int64)
    else:
        # carve-out 20% do treino para avaliação
        eval_frac = 0.20
        ...
        # Reajusta o conjunto de treino escalado ao carve-out
        X_all = scaler.transform(X_tr_o).astype(np.float32)
        y_all = y_tr_i
    ...
    def _eval_fn(m: 'CWGAN_GP'):
        # Amostra sintético (em escala [0,1])
        ...
        # Inverte escala para ORIGINAL e arredonda inteiros/binárias
        ...
        # TSTR em ORIGINAL (com StandardScaler interno)
        scaler_t = StandardScaler().fit(X_syn_orig)
        Xtr = scaler_t.transform(X_syn_orig)
        Xte = scaler_t.transform(X_eval_orig) if len(X_eval_orig) > 0 else X_eval_orig
        clf = LogisticRegression(max_iter=300, multi_class="auto")
        try:
            clf.fit(Xtr, Y_syn)
            y_pred = clf.predict(Xte) if len(X_eval_orig) > 0 else np.array([])
            tstr_macro_f1 = f1_score(y_eval_idx, y_pred, average="macro") if len(X_eval_orig) > 0 else float('nan')
        except Exception as e:
            print(f"[WARN] TSTR falhou: {e}")
            tstr_macro_f1 = float('nan')

        # Baseline real→real em ORIGINAL
        try:
            scaler_b = StandardScaler().fit(X_train_orig)
            Xtr_b = scaler_b.transform(X_train_orig)
            Xte_b = scaler_b.transform(X_eval_orig) if len(X_eval_orig) > 0 else X_eval_orig
            ...
            yb = clf_b.predict(Xte_b) if len(X_eval_orig) > 0 else np.array([])
            baseline_macro_f1 = f1_score(y_eval_idx, yb, average="macro") if len(X_eval_orig) > 0 else float('nan')
        except Exception as e:
        ...
        # KS e correlação em ORIGINAL
        try:
            ks_df = ks_report(X_eval_orig, X_syn_orig, feat_cols) if len(X_eval_orig) > 0 else pd.DataFrame(columns=["feature", "ks_stat"])
            ks_median = float(ks_df["ks_stat"].median()) if not ks_df.empty else float('nan')
        except Exception:
            ks_median = float('nan')
        try:
            corr_fro = corr_gap(X_eval_orig, X_syn_orig) if len(X_eval_orig) > 0 else float('nan')
        except Exception:
            corr_fro = float('nan')
    ...
    y_idx_final = np.random.choice(np.arange(n_classes), size=n_final, replace=True, p=(counts_sm/counts_sm.sum()))
    ...
    with pd.ExcelWriter(out_xlsx, engine='openpyxl') as writer:
        ...
        # sintéticos em ORIGINAL (inteiros/binárias preservadas quando possível)
        ...
        rep = pd.DataFrame({
            "metric": ["n_classes", "train_rows", "holdout_rows", "smote_rows", "device", "oversampler", "feat_final"],
