# v013_defaults: novos padrões solicitados (TSTR early stop, menor SMOTE, maior capacidade, LR assimétricas, inst-noise, amostragem proporcional).
# - --smote-target 2400
# - --hidden 512, --z-dim 128, --n-critic 8, --batch-size 256
# - --lrD 5e-5, --lrG 2e-4
# - --inst-noise 0.03
# - --eval-every 500, --early-patience 4, --min-epochs-early 1500, --tstr-tol 0.3
# - geração sintética e avaliação proporcionais ao histograma real/smote
# - mantém GOLDEN-ONLY, MinMax por feature e arredondamento de binárias/smallint ao salvar
# - DataLoader drop_last=False para evitar zero-batch
...
class CWGAN_GP:
    def __init__(self, x_dim, n_classes, z_dim=128, hidden=512, gp_lambda=10.0, n_critic=8,
                 lrG=2e-4, lrD=5e-5, inst_noise=0.03, device=None):
            ...
            if eval_every and (ep % eval_every == 0):
                ...
                if early_cfg:
                    ...
                    if cur_tstr is not None and not np.isnan(cur_tstr):
                        # tstr_tol em pontos percentuais (pp)
                        if best_tstr is None or (cur_tstr - best_tstr) > (early_cfg.get("tstr_tol", 0.3) / 100.0):
                            best_tstr = cur_tstr
                            plateau = 0
                        else:
                            plateau += 1
                    if (gap_ma == gap_ma) and (early_cfg.get("gap_low", 0.05) <= gap_ma <= early_cfg.get("gap_high", 0.30)):
                        ...
                        stable_gap = 0
                    if ep >= early_cfg.get("min_epochs", 1500) and plateau >= early_cfg.get("patience", 4) and stable_gap >= early_cfg.get("patience", 4):
...
def main():
    ap = argparse.ArgumentParser(description="Hybrid SMOTE + cWGAN-GP (v013_defaults: TSTR-early, histograma, GOLDEN, MinMax, inteiros/0..1)")
    ...
    ap.add_argument("--inst-noise", type=float, default=0.03,
    ...
    ap.add_argument("--tstr-tol", type=float, default=0.3,
    ...
    # DataLoader: TREINO SEMPRE EM X_sm/y_sm; não sobrescreve com carve-out
    ...
    if df_holdout is None:
        ...
        # NENHUMA alteração no loader/treino; avaliação usa X_eval_orig/y_eval_idx
