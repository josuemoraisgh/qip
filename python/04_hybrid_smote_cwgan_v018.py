# v014: cWGAN-GP para tabular condicional com:
# - GOLDEN-ONLY, MinMax por feature, arredondamento de binárias/smallint na saída
# - Oversampling (SMOTE/ROS) e target menor por padrão (1200) para evitar interpolação agressiva
# - Avaliação TSTR/KS/correlação periódica
# - Early stop por plateau E por QUEDA de TSTR, com checkpoint/restauração do melhor por TSTR
# - Regularizadores no G: matching de correlação por mini-batch (Frobenius) e matching de médias (L1)
# Padrões desta versão:
#   --smote-target 1200
#   --batch-size 256, --z-dim 128, --hidden 512, --n-critic 5
...
def fit_resample_balanced(X, y, n_classes, seed=42, target_total=1200):
    ...
    if cur_total < target_total:
        ...
        # distribute approximately uniform, favorecendo classes menores no momento
...
def _batch_corr(x: torch.Tensor) -> torch.Tensor:
    """Correlação por mini-batch (cov padronizada). x: [B,F] em [0,1]."""
    if x.dim() != 2:
        x = x.view(x.size(0), -1)
...
class CWGAN_GP:
    def __init__(self, x_dim, n_classes, z_dim=128, hidden=512, gp_lambda=10.0, n_critic=5,
        ...
        self.topk = []  # lista de dicts: {'tstr': float, 'epoch': int, 'path': str, 'metrics': dict}
        self.checkpoint_dir = None
        self.keep_topk = 3
        self.save_checkpoints = True

    
    def _make_ckpt_dir(self, base_dir):
        os.makedirs(base_dir, exist_ok=True)
        return base_dir

    def _save_ckpt_files(self, epoch, tstr_val, metrics, base_dir, tag):
        # Monta informações e salva num .pt + atualiza topk.json
        fname = f"ckpt_{tag}_ep{epoch:05d}_tstr{tstr_val*100:.2f}.pt"
        fpath = os.path.join(self._make_ckpt_dir(base_dir), fname)
        payload = {
            "epoch": int(epoch),
            "tstr_macro_f1": float(tstr_val),
            "metrics": metrics,
        ...
        try:
            ...
            print(f"[WARN] Falha ao salvar checkpoint em {fpath}: {e}", flush=True)
            return None
        return fpath

    def _update_topk(self, epoch, tstr_val, metrics, base_dir):
        # salva checkpoint, atualiza lista top-k e remove extras
        fpath = self._save_ckpt_files(epoch, tstr_val, metrics, base_dir, tag="tstr")
        if fpath is None:
            return

        self.topk.append({"epoch": int(epoch), "tstr": float(tstr_val), "path": fpath, "metrics": metrics})
        # ordena por TSTR desc e mantém top-k
        self.topk.sort(key=lambda d: d["tstr"], reverse=True)
        if len(self.topk) > self.keep_topk:
            for to_remove in self.topk[self.keep_topk:]:
                try:
                    os.remove(to_remove["path"])
                except Exception:
                    pass
            self.topk = self.topk[:self.keep_topk]

        # grava metadados topk em JSON
        meta = {"topk": self.topk}
        try:
            with open(os.path.join(self._make_ckpt_dir(base_dir), "checkpoints_topk.json"), "w", encoding="utf-8") as fh:
                json.dump(meta, fh, ensure_ascii=False, indent=2)
        except Exception as e:
            print(f"[WARN] Falha ao salvar checkpoints_topk.json: {e}", flush=True)
    ...
    def train(self, loader, n_classes, epochs=2000, log_every=100,
        ...
        # diretório base para checkpoints, se habilitado
        base_ckpt_dir = None
        ...
        for ep in range(1, epochs + 1):
            ...
            for x_real, y_idx in loader:
                ...
                # Passo do gerador (com regularizadores)
                z = torch.randn(x_real.size(0), self.z_dim, device=self.device)
                x_gen = self.G(z, y_oh)          # [B,F] em [0,1]
                xg = self._add_instance_noise(x_gen)
                g_loss = -self.D(xg, y_oh).mean()

                # --- Regularizadores de estrutura conjunta (batch-level) ---
                F = x_real.size(1)
                k = min(F, self.corr_maxfeat if self.corr_maxfeat is not None else F)
                if k > 0:
                    idx_cols = torch.randperm(F, device=self.device)[:k]
                    C_r = _batch_corr(x_real[:, idx_cols].detach())
                    C_f = _batch_corr(x_gen[:, idx_cols])
                    L_corr = torch.norm(C_f - C_r, p='fro') / math.sqrt(k)  # normaliza pela dimensão
                else:
                    L_corr = torch.tensor(0.0, device=self.device)

                mu_r = x_real.mean(dim=0).detach()
                mu_f = x_gen.mean(dim=0)
                L_mu = F.l1_loss(mu_f, mu_r, reduction='mean') if hasattr(F, "l1_loss") else torch.nn.functional.l1_loss(mu_f, mu_r, reduction='mean')
            ...
            if eval_every and (ep % eval_every == 0):
                ...
                if cur_tstr is not None and not np.isnan(cur_tstr):
                    if (self.best["tstr"] is None) or (cur_tstr > self.best["tstr"]):
                        ...
                        # salva 'best' em disco também
                        if self.save_checkpoints and base_ckpt_dir is not None:
                            best_path = self._save_ckpt_files(ep, cur_tstr, metrics, base_ckpt_dir, tag="best")
                    # atualiza top-k em disco
                    if self.save_checkpoints and base_ckpt_dir is not None:
                        self._update_topk(ep, cur_tstr, metrics, base_ckpt_dir)
...
def main():
    ap = argparse.ArgumentParser(description="Hybrid SMOTE + cWGAN-GP (v014: TSTR-early, matching de correlação/médias, GOLDEN, MinMax, inteiros/0..10)")
    ...
    ap.add_argument("--save-checkpoints", action=argparse.BooleanOptionalAction, default=True,
                    help="Se ativo, salva checkpoints do melhor TSTR e mantém top-k no disco.")
    ap.add_argument("--keep-topk", type=int, default=3,
                    help="Quantidade de melhores checkpoints (por TSTR) para manter no disco.")
    ap.add_argument("--checkpoint-dir", type=str, default=None,
                    help="Diretório para salvar checkpoints (default: mesmo diretório do arquivo de saída).")
    ...
    model = CWGAN_GP(
        ...
        corr_maxfeat=args.corr_maxfeat,
        device=device
    )

    # Configura diretório de checkpoints (após instanciar o modelo)
    ckpt_dir = args.checkpoint_dir
    if ckpt_dir is None:
        out_dir = os.path.dirname(args.out) if args.out else os.getcwd()
        ckpt_dir = os.path.join(out_dir, "checkpoints")
    model.checkpoint_dir = ckpt_dir
    model.keep_topk = max(1, int(args.keep_topk))
    model.save_checkpoints = bool(args.save_checkpoints)
    print(f"[INFO] Checkpoints: save={model.save_checkpoints} dir='{model.checkpoint_dir}' topK={model.keep_topk}", flush=True)
