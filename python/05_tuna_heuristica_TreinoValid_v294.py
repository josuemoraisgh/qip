...
# -*- coding: utf-8 -*-
"""
05_tuna_heuristica_TreinoValid.py (v2.9.4 - Comparativo_TopK_Tudo em TDados completo)
Diferenças em relação à v2.9.3:
- A aba 'Comparativo_TopK_Tudo' agora é gerada aplicando a Pontuação_Tunada diretamente sobre TODAS as linhas
  de 'TDados' (sem filtragem por rótulos), e NÃO é mais um espelho de 'Resultado_Heuristica_Tunada'.
- Também ajusta os snapshots para o mesmo comportamento.
Resto: ST continua dentro do softmax (sem pós-regra).
"""
...
def main(args):
    ...
    df_all_full = pd.read_excel(INPUT, sheet_name=ABA_DADOS)   # cópia completa (sem filtros) — usada no Comparativo_TopK_Tudo
    df_all = df_all_full.copy()                                # cópia de trabalho (pode ser filtrada)
    ...
    # Matriz numérica (filtrável)
    X_all = df_all[cols_dados].apply(pd.to_numeric, errors="coerce").fillna(0.0).values
    # Matriz numérica COMPLETA (para Comparativo_TopK_Tudo)
    ...
    # Parsing de rótulos na CÓPIA filtrável (mantemos a full intacta)
    y_lists_all = parse_multilabel(df_all[COL_ALVO], class_core, normal_label=ST_LABEL)

    # Filtragem somente para o pipeline de treino/validação
    ...
    # Construção de alvos para treino (com ST incluído)
    ...
    # Montar W_aug com bias: shape (m+1, K_aug)
    W0_aug = np.zeros((m+1, K_aug), dtype=float)
    W0_aug[:m, :K0] = W0_eff_core
    b_g0 = logit(args.g_init) - logit(1.0/(K_aug-1))
    W0_aug[m, :K0] = 0.0
    W0_aug[m, K0]  = b_g0
    ...
    # Para Comparativo_TopK_Tudo: matriz COMPLETA (sem filtragem)
    ...
    print(f"[BASE] OBJ_VALID={macro_val0:.3%}  ST_top1={0 if np.isnan(st_v0) else st_v0:.3%}")
    ...
    for it in range(1, MAX_ITERS+1):
        ...
        if it % CHECK_EVERY == 0 or it == 1 or it == MAX_ITERS:
            ...
            macro_val_k1 = macro_topk(y_val_aug, P_val, class_to_idx_aug, idx_to_class_aug, k=1,
                                      st_truth_mode=ST_MODE, st_label=ST_LABEL)
            obj_val = macro_val_k1 if TOPK==1 else macro_topk(y_val_aug, P_val, class_to_idx_aug, idx_to_class_aug, k=TOPK,
                                      st_truth_mode=ST_MODE, st_label=ST_LABEL)
            ...
            diag_rows.append({
                "iter": it, "obj_VALID": obj_val,
                "macro_top1_VALID": macro_val_k1,
            ...
            print(f"[IT {it:05d}] OBJ_VA={obj_val:.3%}  ST_top1={0 if np.isnan(st_v) else st_v:.3%}  bias_ST={W[-1,K0]:+.3f}  gamma_eff≈{gamma_eff:.3f}  best={best_score:.3%}")

            if improved:
                carimbo = datetime.now().strftime("%Y%m%d_%H%M%S")
                base_dir = os.path.dirname(OUTPUT) if (OUTPUT and os.path.dirname(OUTPUT)) else os.path.dirname(INPUT)
                base_dir = base_dir or "."
                snap_name = f"best_softmaxST_{int(round(obj_val*10000)):04d}_{carimbo}.xlsx"
                snap_path = os.path.join(base_dir, snap_name)

                saved_snap, va3_saved = _snapshot_save(INPUT, snap_path, best_W,
                                                       df_all_full, df_all, cols_dados, class_core, class_names_aug, split,
                                                       Xb_all_full, Xb_all, Xb_val, y_val_aug,
                                                       class_to_idx_aug, idx_to_class_aug,
                                                       ABA_PONTOS_TUNADA, ABA_RES_HEUR_TUN, ABA_MET_HEUR_TUN, ABA_REGRAS_NORMAL,
                                                       ABA_EXPLICAO, ABA_DIAG, ABA_COMPARATIVO_TUDO, diag_rows, K0, args, mask_w_all_zero, W0_aug, TOPK)
                snapshot_count += 1; last_va3_saved = va3_saved
                print(f"[SNAPSHOT] '{snap_name}' salvo (macro_top{TOPK}={va3_saved:.3%})")
    ...
    P_all = softmax_rows(Xb_all @ W_tuned)
    P_all_full = softmax_rows(Xb_all_full @ W_tuned)  # <- COMPLETO (para Comparativo_TopK_Tudo)

    # VALID detalhado
    P_val = softmax_rows(Xb_val @ W_tuned)
    ...
    # ----- Abas -----
    # Pontuação_Tunada (apenas core weights, sem bias)
    df_pont_tun = pd.DataFrame(W_tuned[:m, :K0].T, columns=cols_dados)
    df_pont_tun.insert(0, "Tipo de Transtorno", class_core)

    # Resultado_Heuristica_Tunada (trabalha só com o CONJUNTO filtrado)
    ...
    # Comparativo_TopK_Tudo (AGORA COM TDados COMPLETO)
    ...
    df_met_sum = pd.DataFrame([{
        ...
        "observacao": "Macro VALID com ST no softmax; Comparativo_TopK_Tudo = TDados completo."
    }])
    ...
    df_expl_add = pd.DataFrame([{"Aba": ABA_RES_HEUR_TUN, "Descricao": "ST no softmax; Comparativo_TopK_Tudo em TDados completo."}])
...
def _snapshot_save(base_input_path, output_path, W_best,
                   df_all_full, df_all_filtered, cols_dados, class_core, class_names_aug, split,
                   Xb_all_full, Xb_all_filtered, Xb_val, y_val_aug,
                   class_to_idx_aug, idx_to_class_aug,
                   ABA_PONTOS_TUNADA, ABA_RES_HEUR_TUN, ABA_MET_HEUR_TUN, ABA_REGRAS_NORMAL,
                   ABA_EXPLICAO, ABA_DIAG, ABA_COMPARATIVO_TUDO, diag_rows, K0, args, mask_w_all_zero, W0_aug, TOPK):
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)

    P_all_filtered = softmax_rows(Xb_all_filtered @ W_best)  # para Resultado_Heuristica_Tunada
    P_all_full     = softmax_rows(Xb_all_full @ W_best)      # para Comparativo_TopK_Tudo
    P_val          = softmax_rows(Xb_val @ W_best)

    macro_val_topk = macro_topk(y_val_aug, P_val, class_to_idx_aug, idx_to_class_aug, k=TOPK,
                                st_truth_mode=args.st_truth_mode, st_label=args.normal_label)

    m = Xb_all_full.shape[1]-1
    df_pont_tun = pd.DataFrame(W_best[:m, :K0].T, columns=cols_dados)
    df_pont_tun.insert(0, "Tipo de Transtorno", class_core)

    # Resultado_Heuristica_Tunada (filtrado)
    df_all_f = df_all_filtered.copy()
    df_res = df_all_f[[df_all_f.columns[0]]].copy()
    if args.col_alvo in df_all_f.columns: df_res[args.col_alvo] = df_all_f[args.col_alvo]
    for j, name in enumerate(class_names_aug): df_res[f"p_{name}"] = P_all_filtered[:, j]
    order_all_aug = np.argsort(-P_all_filtered, axis=1)
    tops_rec = []
    for i in range(P_all_filtered.shape[0]):
        rec = {}
        for t in range(min(3, P_all_filtered.shape[1])):
            c = order_all_aug[i, t]
            rec[f"top{t+1}_classe"] = class_names_aug[c]
            rec[f"top{t+1}_prob"]   = float(P_all_filtered[i, c])
        tops_rec.append(rec)
    df_res = pd.concat([df_res, pd.DataFrame(tops_rec)], axis=1)

    # Comparativo_TopK_Tudo (COMPLETO)
    df_full = df_all_full.copy()
    df_comp = df_full[[df_full.columns[0]]].copy()
    if args.col_alvo in df_full.columns: df_comp[args.col_alvo] = df_full[args.col_alvo]
    for j, name in enumerate(class_names_aug): df_comp[f"p_{name}"] = P_all_full[:, j]
    order_full_aug = np.argsort(-P_all_full, axis=1)
    tops_full = []
    for i in range(P_all_full.shape[0]):
    ...
    df_met = pd.DataFrame([{"macro_topk_VALID": float(macro_val_topk)}])

    df_expl_add = pd.DataFrame([{"Aba": ABA_RES_HEUR_TUN, "Descricao": "Snapshot (ST no softmax); Comparativo_TopK_Tudo em TDados completo."}])

    saved_path = save_preserving_sheets(output_path,
        [(df_pont_tun, ABA_PONTOS_TUNADA),
         (df_res, ABA_RES_HEUR_TUN),
         (df_met, ABA_MET_HEUR_TUN),
         (pd.DataFrame([{"bias_ST": float(W_best[-1, K0]), "gamma_eff_VALID_mean": float(P_val[:, -1].mean())}]), ABA_REGRAS_NORMAL),
         (df_expl_add, ABA_EXPLICAO),
         (pd.DataFrame(diag_rows), ABA_DIAG),
         (df_comp, ABA_COMPARATIVO_TUDO)])
    return saved_path, float(macro_val_topk)
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="ST no softmax; Comparativo_TopK_Tudo = TDados completo; gradiente proximal; snapshots.")
