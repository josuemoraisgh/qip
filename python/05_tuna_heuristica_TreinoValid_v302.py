# -*- coding: utf-8 -*-
"""
05_tuna_heuristica_TreinoValid.py (v3.2.0)
...
def main(args):
    ...
    for it in range(1, MAX_ITERS+1):
        ...
        # ---------- Data augmentation no TREINO (apenas para gradiente) ----------
        Xb_noise = Xb_train.copy()

        # Ruído gaussiano nas features (não no bias)
        if args.input_noise_sigma > 0:
            g = np.random.default_rng(RANDOM_STATE + it).normal(0.0, args.input_noise_sigma, size=Xb_noise[:, :-1].shape)
            Xb_noise[:, :-1] = np.clip(Xb_noise[:, :-1] + g, 0.0, 1.0)

        # Feature-dropout (mask por iteração)
        if args.feature_dropout > 0:
            rng_fd = np.random.default_rng(RANDOM_STATE + 1234 + it)
        ...
        # Passo proximal com projeção em [-1,1]
        ...
        if args.mutations and (it % args.mutation_every == 0):
            ...
            for _try in range(args.mutation_max_tries):
                ...
                # critério: aceitar mutação se EARLY melhorar (mais seguro) ou não piorar (menos conservador)
                if ea_mut > base_ea + 1e-9:  # escolha padrão: precisa melhorar
                    W = W_mut
        ...
        if it % CHECK_EVERY == 0 or it == 1 or it == MAX_ITERS:
            ...
            # Early stop: sem melhora em EARLY
    ...
    df_met_sum = pd.DataFrame([{
        ...
        "observacao": "best_W selecionado por EARLY; TEST é hold-out final; mutações aceitas por melhoria em EARLY."
    }])
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Softmax + W∈[-1,1] + split train/early/test + mutações guiadas por EARLY + anti-overfit.")
    ...
    p.add_argument("--mutation-max-tries", type=int, default=12)
