# -*- coding: utf-8 -*-
"""
05_tuna_heuristica_TreinoValid_vGA.py (v4.0.0)
- Split 1/3-1/3-1/3: TRAIN / EARLY (validação parcial) / TEST (hold-out final)
- best_W e early-stop guiados por EARLY (validação parcial)
- Mutações habilitadas e aceitas somente se EARLY melhorar (ou não piorar, se ajustado)
- Pesos W sempre projetados em [-1, 1] (inclui bias)
- Anti-overfitting: label smoothing, ruído em X e feature-dropout somente no passo de gradiente do TREINO
- NOVO: Modo GA (Genetic Algorithm) com 3 crossovers (uniforme, por-coluna, BLX-α) e mutação auto-adaptativa (σ por indivíduo)
- Modo Memético (GA + poucos passos de gradiente local por indivíduo filho)
"""
...
def main(args):
    ...
    def sgd_train():
        ...
        return best_W, best_score_tr, best_score_ea, diag_rows, total_checks
    ...
    def crossover_columnwise(WA, WB, rng):
        # escolhe por coluna (classe) quem doa a coluna inteira
        m1,k1 = WA.shape
    ...
    # Fitness do GA (maximizar): EARLY_macro_topk - λ||W-W0||^2 - α max(0, TR - EARLY)
    def fitness_of(W):
    ...
    def local_refine_memetic(W_init, steps, rng):
        ...
        for s in range(steps):
            ...
            # aceita somente se EARLY não piorar
            if ea_new >= best_ea - 1e-9:
                Wc, best_tr, best_ea = Wc_new, tr_new, ea_new
        return Wc, best_tr, best_ea
    ...
    def ga_train():
        ...
        pop_W = []
        pop_sigma = []
        # sementes: W0, mais variações leves
        ...
        # completa com indivíduos aleatórios em [-1,1]
        while len(pop_W) < P:
        ...
        print("[GA] base EARLY =", f"{best_ea:.3%}", "| best TR =", f"{best_tr:.3%}")
        print(f"[INFO] tamanhos (TR/EARLY/TEST)=({Xb_train.shape[0]}/{Xb_early.shape[0]}/{Xb_test.shape[0]})")
        ...
        for gen in range(1, args.ga_gens+1):
            ...
            # log
            print(f"[GA GEN {gen:03d}] best EARLY={eas[0]:.3%}  TR={trs[0]:.3%}  fitness={fits[0]:.6f}")

            # early stop populacional
            improved = False
            if eas[0] > best_ea + 1e-6:
                best_ea = eas[0]; best_tr = trs[0]; best_fit = fits[0]; best_W = pop_W[0].copy()
                improved = True; no_improve_gens = 0
            else:
                no_improve_gens += 1

            total_checks += 1
            diag_rows.append({"gen": gen, "best_TR": float(trs[0]), "best_EARLY": float(eas[0]), "best_fit": float(fits[0]), "improved": improved})

            if no_improve_gens >= args.ga_early_stop_gens:
                print("[PARAR] GA early stop: sem melhora em EARLY por", args.ga_early_stop_gens, "gerações."); break
            ...
            while len(new_W) < P:
                # seleção por torneio
                def tournament_pick():
                    idxs = rng.integers(0, P, size=tournament_k)
                    best_i = idxs[0]
                    best_f = fits[idxs[0]]
                    for j in idxs[1:]:
                        if fits[j] > best_f:
                ...
                if cx_type == 0:
                    ...
                    child = crossover_blx_alpha(WA, WB, rng, alpha=blx_alpha)
                ...
                # refinamento local (memético): poucos passos de gradiente; aceitar se EARLY não piorar
                W_ref, tr_ref, ea_ref = local_refine_memetic(child_mut, args.ga_memetic_steps, rng)
        ...
        # fim GA
        return best_W, best_tr, best_ea, diag_rows, total_checks

    # ----------------- EXECUÇÃO: GA ou SGD -----------------
    if args.ga:
        best_W, best_score_tr, best_score_ea, diag_rows, total_checks = ga_train()
    else:
        best_W, best_score_tr, best_score_ea, diag_rows, total_checks = sgd_train()
    ...
    df_met_sum = pd.DataFrame([{
        ...
        "observacao": ("best_W selecionado por EARLY; TEST é hold-out final; "
                       "SGD puro ou GA (com crossovers uniforme/coluna/BLX-α, mutação auto-adaptativa, "
                       "e refinamento memético por gradiente).")
    }])
    ...
    df_regras = pd.DataFrame([
        ...
        {"param": "mutations (SGD)", "value": bool(args.mutations)},
        {"param": "mutation_every (SGD)", "value": int(args.mutation_every)},
        {"param": "mutation_sigma (SGD)", "value": float(args.mutation_sigma)},
        {"param": "mutation_max_tries (SGD)", "value": int(args.mutation_max_tries)},
    ...
    saved_path = save_preserving_sheets(OUTPUT,
        [(df_pont_tun, ABA_PONTOS_TUNADA),
         ...
         (df_comp, ABA_COMPARATIVO_TopK_Tudo if 'ABA_COMPARATIVO_TopK_Tudo' in globals() else ABA_COMPARATIVO_TUDO)])
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Softmax + W∈[-1,1] + split train/early/test + GA (uniforme/coluna/BLX-α) + mutação auto-adaptativa + refinamento memético.")
    ...
    p.add_argument("--ga-pop", type=int, default=48, help="tamanho da população")
    p.add_argument("--ga-gens", type=int, default=15000, help="número máximo de gerações")
    p.add_argument("--ga-elite-frac", type=float, default=0.05, help="fração elitista (mantida intacta em cada geração)")
    p.add_argument("--ga-tournament-k", type=int, default=4, help="tamanho do torneio para seleção de pais")
    ...
    p.add_argument("--ga-cx-prob-blx", type=float, default=0.2, help="probabilidade de crossover BLX-α/aritmético")
    ...
    p.add_argument("--ga-memetic-steps", type=int, default=3, help="passos de gradiente local por filho (memetic refinement)")
    p.add_argument("--ga-early-stop-gens", type=int, default=1200, help="geraçōes sem melhora em EARLY para parar o GA")
