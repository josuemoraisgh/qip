...
def project_bounds(W, adjustable_mask, W0, eps=1e-6):
    ...
    Wp[adjustable_mask] = np.clip(Wp[adjustable_mask], -1.0, 1.0)
    return Wp
...
def main(args):
    ...
    # Normalizações
    X_all = np.clip(np.nan_to_num(X_all, nan=0.0, neginf=0.0, posinf=0.0), 0.0, 10.0)
    X_all_full = np.clip(np.nan_to_num(X_all_full, nan=0.0, neginf=0.0, posinf=0.0), 0.0, 10.0)
    ...
    W0_eff_core = np.clip(W0_sheet.copy(), -1.0, 1.0)
