# -*- coding: utf-8 -*-
"""
v4_fix_autoStop (overfit+multi-restart + early-stop): TDados_clean no treino/val, Etapa 3 robusta
-------------------------------------------------------------------------------------------------
Incrementos sobre a v4:
- **Early-Stop por desempenho**: encerra automaticamente o *restart* atual se atingir
  `macroK@Top3 >= PERF_TARGET` **e** `bad_top3==0` por `EARLY_PATIENCE` checagens consecutivas
  (mantendo a tecla 'q' como bypass manual).
- **Guarda de tempo**: `MAX_TIME_SECONDS_PER_RESTART` limita a duração de cada *restart*.
- **Logs resumidos finais**: imprime Macro@Top3 de TREINO/VALIDAÇÃO com os parâmetros da Etapa 3.

Observação: se você ainda estiver vendo log com "[BURST]" e "drift_cap=0.50", está
rodando uma versão antiga (v3). Use este arquivo.
"""
...
# Early-stop & time guard
AUTO_BREAK_AT_PERF = True
PERF_TARGET = 0.999    # macroK@Top3 >= 99.9%
EARLY_PATIENCE = 20    # nº de checagens consecutivas (CHECK_EVERY) com critério atendido
MIN_ITERS_BEFORE_EARLY = 120  # evita parar cedo demais
MAX_TIME_SECONDS_PER_RESTART = 120  # 0 para desativar
...
def train_one_restart(seed):
    ...
    while it < ITERS_PER_RESTART:
        ...
        if it % CHECK_EVERY == 0:
            ...
            # Early-stop por desempenho
            if macro_now >= PERF_TARGET and bad_rate == 0.0:
                ok_streak += 1
            else:
                ok_streak = 0
            if (AUTO_BREAK_AT_PERF and it >= MIN_ITERS_BEFORE_EARLY and ok_streak >= EARLY_PATIENCE):
                print(f"🟢 Early-stop: macroK@Top{TOPK}>={PERF_TARGET:.3f} e bad_top3=0 por {EARLY_PATIENCE} checagens. Encerrando restart.")
                break

        # tecla para sair (bypass manual)
        ...
        # guarda de tempo
        if MAX_TIME_SECONDS_PER_RESTART > 0 and (time.time() - t0) > MAX_TIME_SECONDS_PER_RESTART:
            print(f"⏱️  Tempo limite por restart ({MAX_TIME_SECONDS_PER_RESTART}s) atingido. Encerrando este restart.")
            break
...
# impressões finais úteis
macro_tr_final = float(df_met_sum_tr.iloc[0]["macro_top3"])
macro_va_final = float(df_met_sum_va.iloc[0]["macro_top3"])
print(f"[TREINO] Macro@Top{TOPK} (com 'Sem Transtorno', T1/T2/γ da etapa 3): {macro_tr_final:.3%}")
print(f"[VALID ] Macro@Top{TOPK} (com 'Sem Transtorno', T1/T2/γ da etapa 3): {macro_va_final:.3%}")
