# -*- coding: utf-8 -*-
"""
Treino com 'Sem Transtorno' COMO CLASSE (K+1) — Split garantido
---------------------------------------------------------------
- 'Sem Transtorno' é treinada e validada como qualquer outra classe.
- O split garante que:
   * Se a classe tiver m==1 exemplos, esse único exemplo vai para **TREINO** (para permitir aprendizado).
   * Se m>=2, tenta dividir aproximadamente meio a meio.
   * Pós-ajuste: se alguma classe (com m>=2) ficar sem exemplos em TREINO ou VALID, movemos 1 exemplo para equilibrar.
- Guard-rails idênticos aos anteriores (máscara por entrada; ST só pode usar features já ativas e com sinal).

Demais abas e lógica permanecem iguais à v1.
"""
...
def iterative_half_split_known(y_lists, class_names_all, seed=42):
    ...
    for c in class_names_all:
        ...
        if m == 0: 
            ...
            train_quota[c] = 1     # <--- garantir treino quando há 1 exemplo
        else: 
    ...
    # Se uma classe com m>=2 ficou sem TREINO ou sem VALID, mova 1 exemplo do outro lado.
...
df_regras = pd.DataFrame([
    ...
    {"param": "observacao", "value": "Classe 'Sem Transtorno' treinada no W (sem regra T1/T2/γ)."},
])
