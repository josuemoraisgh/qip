...
def main():
    ...
    # ---------- Métricas (para as linhas FILTRADAS) ----------
    # y_lists_all contém todas as linhas; filtramos para y_lists_filtrado
    if COL_ALVO in df_dados_full.columns:
        y_lists_filtrado = [labs for labs, keep in zip(y_lists_all, keep_nonempty) if keep]
    else:
        y_lists_filtrado = [[] for _ in range(P_filtrado.shape[0])]

    def macro_topk(y_lists, proba, class_names, k=3):
        ...
        for c_idx, c_name in enumerate(class_names):
            ...
            if sup == 0:
                continue
            hits = sum(c_idx in order[i] for i in np.where(mask)[0])
            accs.append(hits / sup)
        return float(np.mean(accs)) if accs else 0.0

    macro_k1 = macro_topk(y_lists_filtrado, P_filtrado, class_core, k=1)
    macro_k2 = macro_topk(y_lists_filtrado, P_filtrado, class_core, k=2)
    macro_k3 = macro_topk(y_lists_filtrado, P_filtrado, class_core, k=3)

    rows_cls = []
    order3 = np.argsort(-P_filtrado, axis=1)[:, :3]
    for c_idx, c_name in enumerate(class_core):
        mask = np.array([c_name in labs for labs in y_lists_filtrado], bool)
        sup = int(mask.sum())
        if sup == 0:
            rows_cls.append({"classe": c_name,
                             "top1_rate": np.nan,
                             "top2_rate": np.nan,
                             "top3_rate": np.nan,
                             "acertos_top1": 0, "acertos_top2": 0, "acertos_top3": 0,
                             "suporte": 0})
            continue
        idxs = np.where(mask)[0]
        ord1 = np.argsort(-P_filtrado[mask], axis=1)[:, :1]
        ord2 = np.argsort(-P_filtrado[mask], axis=1)[:, :2]
        ord3_c = np.argsort(-P_filtrado[mask], axis=1)[:, :3]
        h1 = sum(c_idx in ord1[r] for r in range(ord1.shape[0]))
        h2 = sum(c_idx in ord2[r] for r in range(ord2.shape[0]))
        h3 = sum(c_idx in ord3_c[r] for r in range(ord3_c.shape[0]))
        rows_cls.append({"classe": c_name,
                         "top1_rate": h1/sup, "top2_rate": h2/sup, "top3_rate": h3/sup,
                         "acertos_top1": h1, "acertos_top2": h2, "acertos_top3": h3,
                         "suporte": sup})
    df_metricas_result = pd.concat([
        pd.DataFrame([{"secao":"agregado_RESULT",
                       "macro_top1": macro_k1, "macro_top2": macro_k2, "macro_top3": macro_k3,
                       "macro_topK": macro_k3}]),
        pd.DataFrame(rows_cls).assign(secao="por_classe_RESULT")
    ], ignore_index=True)
    ...
    try:
        with pd.ExcelWriter(OUTPUT, engine="openpyxl", mode=("a" if os.path.exists(OUTPUT) else "w"),
            ...
            df_metricas_result.to_excel(w, sheet_name="Metricas_Resultados", index=False)
