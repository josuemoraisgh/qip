...
LAMBDA_L2         = 5e-2
LR                = 0.05
MAX_DRIFT         = 0.05 # cada elemento de W só pode variar ±0.05 a partir de W0
MAX_STEP          = 0.01  # variação máxima por iteração (elemento a elemento)
MAX_ITERS         = 800
CHECK_EVERY       = 10
TARGET_MACRO_TOP3 = 0.99
EPS_W             = 1e-6
RANDOM_STATE      = 42
NO_IMPROVE_PATIENCE = 8  # novo: para após 8 checagens sem melhora

# Grid para aprender a regra do "Sem Transtorno"
GRID_T1 = np.linspace(0.55, 0.75, 9)   # limiar p/ top1_prob
GRID_T2 = np.linspace(0.16, 0.26, 11)  # limiar p/ (top1 - top2)
GRID_G  = np.linspace(0.10, 0.40, 7)   # fração de prob. alocada ao "Sem Transtorno"
# ============================================
...
def project_bounds(W, adjustable_mask, W0, eps=1e-6):
    """
    Projeção:
      - colunas NÃO ajustáveis -> exatamente W0
      - colunas ajustáveis     -> clip para [eps, 1] e também para [W0 - MAX_DRIFT, W0 + MAX_DRIFT]
    """
    Wp = W.copy()
    # congela não-ajustáveis
    Wp[~adjustable_mask, :] = W0[~adjustable_mask, :]

    if np.any(adjustable_mask):
        Wa = Wp[adjustable_mask, :]
        W0a = W0[adjustable_mask, :]
        # limites por elemento ao redor de W0
        low  = np.maximum(eps, W0a - MAX_DRIFT)
        high = np.minimum(1.0, W0a + MAX_DRIFT)
        Wa = np.minimum(np.maximum(Wa, low), high)
        Wp[adjustable_mask, :] = Wa
