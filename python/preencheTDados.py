...
if __name__ == '__main__':
    ...
    df['Tela 02_part_2'] = pd.to_datetime((df['Tela 02_part_3'].astype(str).str.strip() + " " + df['Tela 02_part_2'].astype(str).str.strip()),format='%d/%m/%Y %H:%M', errors="coerce").sub(timeInicial).dt.total_seconds()
    df['Tela 02_part_2'] = (df['Tela 02_part_2'].abs() > 120).astype(int) # se ele errou apenas em 3 minutos ou menos, considera-se que ele acertou o dia
    df['Tela 02_part_3'] = (pd.to_datetime(df['Tela 77_part_1'],format='%Y-%m-%d %H:%M:%S.%f',errors='coerce').sub(timeInicial).dt.total_seconds().abs() > 120).astype(int) 
